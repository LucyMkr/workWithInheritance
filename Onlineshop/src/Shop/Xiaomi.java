package Shop;

public class Xiaomi extends Phone{
    Xiaomi(int costPrice, int price, int dataOfManufacture, int RAM, int memory) {
        super(costPrice, price, dataOfManufacture, RAM, memory);
    }

    @Override
    protected int setCostPrice(int costPrice) {
        if (costPrice >= 100000 && costPrice <= 700000) {
            this.costPrice = costPrice;
        } else {
            System.exit(20);
        }
        return costPrice;
    }

    @Override
    protected int setPrice(int price) {
        if (price >= 150000 && price <= 300000 && RAM <= 8 && memory <= 256 || price > 300000 && price <= 700000 && RAM > 8 && memory > 256) {
            this.price = price;
        } else {
            System.exit(21);
        }
        return price;
    }

    @Override
    public void creditSum(int monthsOfCredits) {
        super.creditSum(monthsOfCredits);
        if (monthsOfCredits == 12) {
            price *= 1.05;
        } else if (monthsOfCredits == 24) {
            price *= 1.1;
        } else {
            System.exit(22);
        }

    }
}
